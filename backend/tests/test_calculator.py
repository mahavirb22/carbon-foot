"""Unit tests for the pure carbon footprint calculation engine."""
...
def test_empty_input_yields_diet_only_footprint():
...
def test_car_emissions_annualised_correctly():
...
def test_electric_car_emits_less_than_petrol():
...
def test_home_energy_divided_by_household_size():
...
def test_total_equals_breakdown_sum_and_tonnes_consistent():
    result = calculate_footprint(
        CarbonInput(
...
def test_comparison_ratios_are_correct():
    result = calculate_footprint(CarbonInput(diet=factors.DietType.VEGAN))
    c = result.comparison
