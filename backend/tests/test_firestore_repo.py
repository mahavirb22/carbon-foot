"""Tests for the Firestore repository using an in-memory fake client.

The fake mimics the minimal Firestore client API surface the repository relies
on (collection/document chaining, ``set``, ``order_by``/``limit``/``stream``)
so persistence logic is exercised without network access or credentials.
"""
...
def test_listing_scoped_to_device_and_newest_first(repo):
